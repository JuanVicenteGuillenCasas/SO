...
static int my_read(const char * ruta, char * buffer, size_t tam, off_t pos,struct fuse_file_info * inforFichero){
	NodeStruct * informacionNodo = myFileSystem.nodes[inforFichero->fh];
	int a = 0;
	while(a < tam){ 
		char auxiliar[BLOCK_SIZE_BYTES]; // variable auxiliar que va a ir recogiendo bloque por bloque...
		int indiceB = pos / BLOCK_SIZE_BYTES, bytesLeidos;
		off_t salidaLseek;
		salidaLseek = lseek(myFileSystem.fdVirtualDisk, informacionNodo->blocks[indiceB] * BLOCK_SIZE_BYTES ,SEEK_SET); //Colocas el disco virtual en la posición donde empiezas a leer.
			if(salidaLseek == (off_t)-1) return -1;
		bytesLeidos = read(myFileSystem.fdVirtualDisk, &auxiliar , BLOCK_SIZE_BYTES); // Lees BLOCK_SIZE_BYTES bytes.
			if(bytesLeidos == -1 ) return -1;
		if(bytesLeidos > 0){	
			int b = pos % BLOCK_SIZE_BYTES; // Donde empieza en el bloque a leer el archivo.
			pos += b; // corres la posicion hasta donde empiece a leer el archivo y ya a partir de ahi la posicion se corre con el bucle.(Moviendose pos con los bytes leidos con read())
			while(a < tam && b < BLOCK_SIZE_BYTES){
				buffer[a] = auxiliar[b];
				b++; a++; pos++;
			}
		}
...
static int my_unlink(const char * fichero){
	int n =  findFileByName(&myFileSystem, fichero + 1); //El primer espacio contiene una '/' que nos sobra por lo que corremos un espacio.	
	if(n == -1)	return -1;
		 
	int nodo = myFileSystem.directory.files[n].nodeIdx;
	NodeStruct * informacionNodo = myFileSystem.nodes[nodo];
	int b = 0;
	while(b < informacionNodo -> numBlocks){
	 myFileSystem.bitMap[informacionNodo->blocks[b]] = 0;
	 b++;
	}	
	myFileSystem.directory.files[n].freeFile = true;
	myFileSystem.directory.numFiles--;
	myFileSystem.numFreeNodes++;
	myFileSystem.superBlock.numOfFreeBlocks += informacionNodo->numBlocks;
	informacionNodo -> freeNode = true;
	informacionNodo -> numBlocks = 0;
	informacionNodo -> fileSize = 0;
	
	updateNode(&myFileSystem,nodo,informacionNodo); //Actualizamos todos los cambios realizados.
	updateBitmap(&myFileSystem);
	updateSuperBlock(&myFileSystem);
	updateDirectory(&myFileSystem);
	
	myFileSystem.nodes[nodo]=NULL; // Dejamos a nulo el valor del nodo raiz.
	free(informacionNodo);
...
struct fuse_operations myFS_operations = {
	...
	.unlink = my_unlink, // Borra el archivo. 
	.read = my_read, // Lee el archivo.
};
